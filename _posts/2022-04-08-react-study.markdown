...
- 사용자 인터페이스 빌드를 위한 자바스크립트 라이브러리(UI 기능만 제공. 프레임워크나 풀스택 솔루션이 아님)
- 전역 상태 관리, 라우팅, 빌드 시스템은 개발자가 직접 구축해야함
...
1. 개념
   - 함수 컴포넌트에 Hook이 도입되면서 클래스 컴포넌트는 componentDidCatch를 사용할 때를 제외하고 모두 함수 컴폰넌트를 사용하여 구현 가능

## Redux

1. 개념
   - 리액트 생태계에서 가장 사용률이 높은 상태관리 라이브러리
   - 상태 고나련 로직들을 다른 파일로 분리하여 관리 가능, 글로벌 상태 관리도 쉬워짐
2. 관련 키워드
   - 액션 : 상태 변화가 필요할 때 발생시키는 객체. type 필드를 필수적으로 가져야함
   ```
   {
     type: "ADD_TODO"
     data : {
   ...
   ```
   - 액션 생성 함수 : 단순 파라미터 받아와서 액션 객체 형태로 만들어줌
   - 리듀서 : 변화를 일으키는 함수. state(현재 상태), action(전달 받은 액션) 두 파라미터를 받아와 새로운 상태 만들어 반환(useReducer에서 쓰는 리듀서와 같은 형태)
   ```
   ...
   // useReducer 에선 default 부분에 throw new Error('Unhandled Action') 과 같이 처리. 리덕스의 리듀서에서는 기존 state를 그대로 반환
   ```
   - 스토어 : 한 애플리케이션 당 하나의 스토어를 만든다. 스토어 안에는 현재 앱 상태, 리듀서, 내장 함수가 들어감
   - 디스패치 : 스토어의 내장함수 중 하나. 액션을 발생시키는 것. dispatch 함수에는 액션을 파라미터로 전달. 함수 호출되면 스토어는 리듀서 함수 실행해서 해당 액션 처리하는 로직이 있다면 액션을 참고해 새로운 상태를 만들어줌
   - 구독 : 스토어의 내장함수. subscribe는 함수를 파라미터로 받아, 액션이 디스패치 되었을때 마다 전달해준 함수가 호출됨
...
- 모듈 번들러. 분리되어있는 자바스크립트 소스 파일을 하나로 묶어줄 수 있음
